...
class RecipeView extends View {
  ...
  addHandlerUpdateServings(handler) {
    this._parentElement.addEventListener('click', function (e) {
      const clickedServingSizeChangeButton = e.target.closest(
        '.btn--update-servings'
      );

      //guard clause
      if (!clickedServingSizeChangeButton) return;

      // Determine if it was the plus or minus button that was clicked
      const updateTo = +clickedServingSizeChangeButton.dataset.updateTo; //convert to number

      if (updateTo > 0) handler(updateTo);
    });
  }

  // add handler for bookmark click
  addHandlerAddBookmarkToRecipe(handler) {
    this._parentElement.addEventListener('click', function (e) {
